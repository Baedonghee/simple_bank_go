postgres:
	docker run --name postgres14 --network bank-network -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=mysecret -d postgres:14-alpine
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/Baedonghee/simple_bank_go/db/sqlc Store
